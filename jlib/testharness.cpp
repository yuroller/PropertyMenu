...
char* mainMenuItems[numberOfMainMenuItems] =
{
	"1)Write a string\0"
	,"2)Print a number\0"
	,"3)Color test\0"
	,"4)Formatting Output\0"
	,"5)Read a string\0"
	,"6)Read a number\0"
	,"7)Draw a char box\0"
	,"8)Draw a window with a title\0"
	,"9)Standard Menu\0"
	,"10)Windowed Menu\0"
	,"11)Scrolling Menu\0"
	,"12)Windowed Scrolling Menu\0"
	,"13)Custom Screen Buffers\0"
	,"14)Read a Float\0"
	,"15)Quit (or press Escape)\0"
};
			...
			default:
				{
					...
					if((int)pFunc == numberOfMainMenuItems)
						return;
					pFunc();
				}
				break;
...
void ReadString()
{
	...
	pCore->ScanString(pCore->CursorPosition(),buffer,21);
	pCore->Prints("You wrote: ",FALSE,NULL,1,3);
	pCore->Prints(buffer,TRUE,NULL);
