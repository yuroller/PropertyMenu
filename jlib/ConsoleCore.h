...
public:
	...
	//		Wait
	// Hangs program execution until any key is pressed
	void Wait()
	{
		while(!_kbhit());
		if(_getch() == KB_EXTENDED) // Extended key?! Side effect of _kbhit?
			_getch();
	}
	//		ScanNumber
...
private:
	...
	CHAR_INFO m_screenBuffer[SCREEN_BUFFER_SIZE];	// For saving/loading the screen
